...
// --- Updated from before ---
const pool = new Pool({
  host: process.env.POSTGRES_SERVICE_HOST || process.env.PGHOST || "127.0.0.1",
  port: Number(process.env.POSTGRES_SERVICE_PORT || process.env.PGPORT || 5433),
  user: process.env.POSTGRES_USER || process.env.PGUSER || "postgres",
  password: process.env.POSTGRES_PASSWORD || process.env.PGPASSWORD || "SecurePassword",
  database: process.env.POSTGRES_DB || process.env.PGDATABASE || "eventscape",
});
...
app.post("/users", async (req, res) => {
  try {
    const { email, password_hash = "hash", display_name = null } = req.body || {};
...
app.post("/events", async (req, res) => {
  try {
    const {
      host_id, title, start_time,
      end_time = null, location = null, content = null,
      visibility = "public", capacity = null, waitlist = false,
    } = req.body || {};

    if (!isUUID(host_id) || !title || !start_time) {
      return res.status(400).json({ error: "host_id (uuid), title, start_time are required" });
    }
    if (!VISIBILITY.has(visibility)) {
      return res.status(400).json({ error: "visibility must be public|private|unlisted" });
    }
    ...
    if (e.code === "23503")
      return res.status(400).json({ error: "Invalid host_id (must exist in users)" });
...
app.post("/events/:id/rsvp", async (req, res) => {
  try {
    ...
    const { user_id, status = "interested", waitlist_position = null } = req.body || {};

    if (!isUUID(event_id) || !isUUID(user_id)) {
      return res.status(400).json({ error: "event_id and user_id must be UUIDs" });
    }
    if (!RSVP_STATUS.has(status)) {
      return res.status(400).json({ error: "status must be going|waitlisted|interested|cancelled" });
    }
    ...
    `;
    const { rows } = await pool.query(sql, [event_id, user_id, status, waitlist_position]);
    res.status(201).json(rows[0]);
  } catch (e) {
    if (e.code === "23503")
      return res.status(400).json({ error: "Invalid event_id or user_id (FK constraint)" });
...
app.post("/events/:id/announcements", async (req, res) => {
  try {
    ...
    if (!isUUID(event_id) || !isUUID(host_id) || !content)
      return res.status(400).json({ error: "event_id, host_id (UUID), content required" });
    ...
    if (e.code === "23503") return res.status(400).json({ error: "Invalid event_id or host_id" });
...
app.post("/invitations", async (req, res) => {
  try {
    const { event_id, invited_by, mode, recipient_email = null, message = null } = req.body || {};
    if (!isUUID(event_id) || !isUUID(invited_by) || !mode)
      return res.status(400).json({ error: "event_id, invited_by (UUID), mode required" });
    ...
    if (e.code === "23503") return res.status(400).json({ error: "Invalid event_id or invited_by" });
...
app.listen(port, async () => {
  try {
    ...
    console.log(
      `Server running on http://localhost:${port} (DB: ${process.env.POSTGRES_DB || process.env.PGDATABASE || "eventscape"} @ ${process.env.POSTGRES_SERVICE_HOST || process.env.PGHOST || "127.0.0.1"}:${process.env.POSTGRES_SERVICE_PORT || process.env.PGPORT || 5433})`
    );
  } catch (e) {
